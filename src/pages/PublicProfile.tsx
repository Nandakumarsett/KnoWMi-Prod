...
export default function PublicProfile() {
  ...
  // Friendly persona label (capitalise each word, replace underscores)
  const personaLabel = personaRaw
    .replace(/_/g, ' ')
  ...
  if (!isDesktop) {
    return (
      <div className="min-h-screen bg-[#FDF6EC] text-[#1A1A1A] font-sans selection:bg-orange-500/30 pb-12">
        <ProfileViewTracker profileId={profile.id} />
        
        {/* Top Header */}
        <header className="fixed top-0 w-full z-[120] bg-[#FDF6EC]/80 border-b border-[#E5D5C4] px-6 py-3 backdrop-blur-md flex justify-between items-center">
        ...
        <div className="pt-20 px-6 flex justify-center mb-6">
          <button 
            ...
            className="flex items-center gap-2 px-6 py-3 bg-white hover:bg-neutral-50 border border-[#E5D5C4] rounded-[12px] font-black uppercase tracking-widest text-xs text-[#1A1A1A] transition-all hover:scale-[1.02] active:scale-[0.98] shadow-md shadow-[#C1440E]/5"
          >
            ✦ Tap to view QR
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#FDF6EC] text-[#1A1A1A] font-sans selection:bg-orange-500/30">
      <ProfileViewTracker profileId={profile.id} />
      
      <header className="fixed top-0 w-full z-[120] bg-[#FDF6EC]/80 border-b border-[#E5D5C4] px-8 py-4 backdrop-blur-md">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-3">
             <img src="/logo-square.png" alt="Logo" className="w-8 h-8 rounded-lg object-cover" />
             <span className="text-xs font-black uppercase tracking-[0.3em] opacity-40 text-[#1A1A1A]">
      ...
      <main className="max-w-6xl mx-auto pt-28 pb-12 px-8">
        <div className="flex gap-12 items-start justify-center">
          ...
          <div className="w-[380px] shrink-0 sticky top-28">
            <div className="bg-white rounded-[24px] p-10 shadow-2xl text-[#1A1A1A] border border-[#E5D5C4] flex flex-col items-center">
              ...
              <h1 className="text-4xl font-black tracking-tight mb-2 text-center text-[#1A1A1A]">{safeDisplayName}</h1>
              <div className="flex flex-col items-center mb-6">
                <p className="text-xs font-black uppercase tracking-[0.2em] opacity-30 text-[#1A1A1A]">{profile.member_id}</p>
                {profile.is_verified && (
              ...
              <p className="text-sm text-[#5C5246] text-center leading-relaxed mb-10 italic">
          ...
          <div className="flex-1 max-w-[680px] min-h-[600px] rounded-[24px] overflow-hidden border border-[#E5D5C4] bg-white shadow-2xl p-6">
             <PersonaRouter profile={profile} />
          </div>
      ...
      <footer className="max-w-7xl mx-auto px-8 py-12 border-t border-[#E5D5C4] flex justify-between items-center opacity-40">
        <p className="text-[10px] font-black uppercase tracking-widest text-[#1A1A1A]">© 2024 KnoWMi Identity Platform</p>
        <div className="flex gap-6 text-[10px] font-black uppercase tracking-widest text-[#1A1A1A]">
           <span>Privacy</span>
           <span>Terms</span>
           <span>Protocol</span>
        </div>
