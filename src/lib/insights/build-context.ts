...
export async function buildInsightContext(
  ...
  const yesterdayLocalStr = getLocalDateString(yesterdayDate);
  ...
  let personaData = profileDataObj.persona_data || {};
  if (Array.isArray(personaData.identities) && personaData.identities.length > 0) {
    const activeId = personaData.identities.find((i: any) => i.persona_type === persona || i.active) || personaData.identities[0];
    personaData = activeId.data || {};
  } else if (personaData[persona]) {
    personaData = personaData[persona];
  }
  ...
  const todayScans = views.filter((v: any) => {
    ...
    return getLocalDateString(new Date(v.viewed_at)) === todayLocalStr;
  }).length;

  // Yesterday scans
  const yesterdayScans = views.filter((v: any) => {
    if (!v.viewed_at) return false;
    return getLocalDateString(new Date(v.viewed_at)) === yesterdayLocalStr;
  }).length;
  ...
  for (let i = 0; i < 7; i++) {
    ...
    weekSparkline[i] = views.filter((v: any) => v.viewed_at && getLocalDateString(new Date(v.viewed_at)) === ds).length;
  }
  ...
  scans.forEach((s: any) => {
    ...
    if (ds && getLocalDateString(new Date(ds)) < todayLocalStr) {
  ...
  scans.forEach((s: any) => {
    if (!s.city || s.city === 'Unknown') return;
    const ds = s.scanned_at || s.created_at || s.viewed_at;
    if (ds && getLocalDateString(new Date(ds)) === todayLocalStr && !pastCities.has(s.city)) {
      newCityToday = s.city;
    }
  });
  ...
  views.forEach((v: any) => {
    if (!v.viewed_at) return;
    const d = getLocalDateString(new Date(v.viewed_at));
  ...
  // If no scan today, check if the streak was alive yesterday
  ...
  return {
    ...
    analytics: {
      ...
      repeatScore: totalViews > 0 ? Math.round(((totalViews - uniqueVisitors) / totalViews) * 100) : 0,
      todayScans,
      yesterdayScans,
      thisWeekScans,
      lastWeekScans,
