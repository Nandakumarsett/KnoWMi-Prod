...
export const PATTERN_TEMPLATES: InsightTemplate[] = [

  {
    ...
    condition: ctx => ctx.analytics.peakHour !== null && ctx.analytics.totalQRScans >= 15,
    priority: 1,
...

  {
    ...
    condition: ctx => ctx.analytics.peakDay !== null && ctx.analytics.totalQRScans >= 20,
    priority: 2,
...

  {
    ...
    condition: ctx => ctx.analytics.totalQRScans < 15 && ctx.analytics.totalQRScans > 0,
    priority: 5,
    tokens: ctx => ({
      remaining: String(15 - ctx.analytics.totalQRScans),
      total: String(ctx.analytics.totalQRScans),
    }),
  },
  {
    id: 'pt_starter',
...
export const BEST_TIME_TEMPLATES: InsightTemplate[] = [
  ...
  // Weekend boost
  {
    ...
    condition: ctx => ctx.time.isWeekend && ctx.analytics.totalQRScans < 15,
    priority: 3,
    tokens: () => ({}),
  },
  ...
  // No data yet — encouraging
  {
    ...
    condition: ctx => ctx.analytics.totalQRScans < 15,
    priority: 5,
    tokens: ctx => ({ remaining: String(Math.max(0, 15 - ctx.analytics.totalQRScans)) }),
  },
