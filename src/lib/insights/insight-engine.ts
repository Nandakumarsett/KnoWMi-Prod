...
function selectBestTemplate(
  ...
  // Force show Priority 1 insights without rotating them
  if (eligible[0].priority === 1) {
    return renderTemplate(eligible[0], ctx);
  }

  // Add weekly variety for less critical insights: rotate based on week number
...
export function computeTopNudge(ctx: InsightContext, dismissed: string[] = []): Nudge | null {
  ...
  if (analytics.peakDay === time.dayOfWeek && analytics.totalViews >= 5) {
    nudges.push({
      id: 'best_day_today',
      emoji: '⚡',
...
export function computeBestTime(ctx: InsightContext): BestTimeResult {
  ...
  if (analytics.totalViews < 5 || analytics.peakHour === null) {
    return {
      shouldWearNow: false,
      message: `Keep sharing and wearing! We'll map your peak window after ${Math.max(0, 5 - analytics.totalViews)} more views.`,
      emoji: '📡',
  ...
  const confidence: BestTimeResult['confidence'] =
    analytics.totalViews >= 30 ? 'high' :
    analytics.totalViews >= 10  ? 'medium' : 'low';
