...
import { Plus, Trash2, RefreshCw, FileText, Eye } from 'lucide-react'
...
export function DeveloperForm({ data = {}, onChange, isOwner, onUpload, uploading }: DeveloperFormProps) {
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-x-12 gap-y-24 items-start">
          ...
          {(data.projects || []).map((p: any, i: number) => (
            <div key={i} className="p-8 bg-white border border-[#E5D5C4] rounded-[12px] shadow-sm space-y-8 relative">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {/* Media Upload Area */}
                <div className="relative h-44 rounded-2xl overflow-hidden bg-neutral-50 border-2 border-dashed border-neutral-200 hover:border-[#3B82F6] transition-all flex flex-col items-center justify-center group">
                  {p.url ? (
                    <>
                      ...
                      <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                         <span className="text-[10px] font-black text-white uppercase tracking-widest">Replace Screenshot</span>
                      </div>
                    </>
                  ) : (
                    <>
                      <Plus size={24} className="text-neutral-300 mb-2" />
                      <span className="text-[10px] font-black uppercase text-neutral-400">Project Screenshot</span>
                    </>
                  )}
                  <input 
                    type="file" 
                    ...
                    onChange={(e) => {
                      ...
                      if (file && onUpload) onUpload(file, `developer_project_media_${i}`);
                    }}
                  />
                  {uploading && (
      ...
      <section className="space-y-4">
        <h3 className="text-sm font-black uppercase tracking-widest text-[#1A1A1A]">Section 5: Professional CV</h3>
        <div className="bg-white p-10 rounded-[24px] border border-[#E5D5C4] shadow-sm">
          <div className="flex items-center gap-6 p-6 border-2 border-dashed border-neutral-200 rounded-2xl hover:border-[#3B82F6] hover:bg-blue-50 transition-all group relative">
            <div className="w-12 h-12 bg-neutral-50 rounded-xl flex items-center justify-center text-neutral-400 group-hover:bg-white group-hover:text-[#3B82F6] transition-all">
              <FileText size={24} />
            </div>
            <div className="flex-1">
              <h4 className="text-xs font-black uppercase text-neutral-900 mb-0.5">
                {data.resume_url ? 'CV Uploaded ✓' : 'Upload Developer CV'}
              </h4>
          ...
          {data.resume_url && (
            <a 
              ...
              target="_blank" 
              rel="noreferrer"
              className="inline-flex items-center gap-2 mt-4 text-[10px] font-black uppercase text-[#3B82F6] hover:text-[#2563EB] tracking-widest"
            >
              <Eye size={12} /> View Current CV
            </a>
          )}
        </div>
      </section>
