  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl p-4 flex items-start gap-3"
        ...
        <div>
          <p className="text-sm font-bold text-neutral-200">Policy Change Notification</p>
      ...
      <div className="rounded-xl p-6 bg-[#1a1a1a] border-[3px] border-white rounded-xl shadow-[4px_4px_0px_#fff]">
        <h3 className="font-bold text-sm mb-5 text-white">Compose Policy Broadcast</h3>
        ...
        <form onSubmit={handleSend} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-[10px] font-bold uppercase tracking-wide mb-1.5 text-neutral-400 font-bold">
                Policy
              </label>
              <select
                ...
                className="w-full px-4 py-2.5 rounded-xl text-sm outline-none border-[3px] border-white/20 bg-[#1a1a1a] text-white"
              >
          ...
          <div>
            ...
            <textarea
              ...
              rows={6}
              required
              className="w-full px-4 py-3 rounded-xl text-sm outline-none resize-y border-[3px] border-white/20 bg-[#1a1a1a] text-white"
            />
          </div>
      ...
      <div className="rounded-xl overflow-hidden bg-[#1a1a1a] border-[3px] border-white rounded-xl shadow-[4px_4px_0px_#fff]">
        <div className="px-5 py-4 border-b font-bold text-sm text-white border-white/20">
          Broadcast History
        </div>
        {loadingHistory ? (
          <div className="text-center py-8 text-sm text-neutral-400 font-bold">Loading...</div>
        ) : broadcasts.length === 0 ? (
          <div className="text-center py-8 text-sm text-neutral-400 font-bold">No broadcasts sent yet</div>
        ) : (
          broadcasts.map(b => (
            <div key={b.id} className="flex items-center justify-between p-4 border-b last:border-b-0 border-white/20">
              <div>
                <p className="text-sm font-bold text-white">{b.metadata?.policy_name || b.subject}</p>
