  ...
  return (
    ...
    >
      <div className="max-w-[1200px] mx-auto pl-2 pr-4 md:px-6 flex items-center justify-between lg:justify-start h-20 lg:h-[110px] gap-0 w-full relative">
        {/* Logo */}
        <a href="/" className="flex items-center gap-0 flex-shrink-0 group" aria-label="KnoWMi home">
          <img
            src="/logo-square.png"
            alt="KnoWMi"
            className="h-16 lg:h-20 w-auto object-contain transition-transform duration-300 group-hover:scale-105"
          />
          <div className="flex flex-col leading-none -ml-3 lg:-ml-4">
            <div className="relative">
              <span
                ...
                className="lg:text-[32px] font-black text-[var(--ink)] tracking-tight block leading-[0.9]"
              >
                KnoW<span style={{ color: 'var(--saffron)' }}>M</span>i
              </span>
            </div>
            <span
              className="text-[8px] lg:text-[10px] font-black tracking-[0.15em] text-neutral-400 mt-1 uppercase"
        ...
        <div className="hidden lg:flex items-center gap-1.5 flex-1 justify-center" role="list">
          {navLinks.map(l => {
            const isActive = activeSection === l.href || (l.href.startsWith('/') && !l.href.includes('#') && window.location.pathname === l.href)
            return (
              <a
                key={l.label}
                href={l.href}
