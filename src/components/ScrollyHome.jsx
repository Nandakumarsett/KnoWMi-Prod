...
export default function ScrollyHome() {
  ...
  useEffect(() => {
    ...
    let ctx = gsap.context(() => {
      ...
      // Only run scrollytelling animations on desktop viewports (>= 1024px)
  ...
  return (
    <div ref={containerRef} className="bg-black text-white overflow-hidden relative z-0 pb-16 lg:pb-32 font-display">
      ...
      <section className="hero-section h-[100svh] w-full relative flex items-center justify-center overflow-hidden bg-black">
        ...
        <div className="hero-text relative z-10 text-center px-4 w-full max-w-4xl">
          <p className="text-xs md:text-sm font-black text-orange-500 tracking-[0.3em] uppercase mb-4 md:mb-6">Introducing KnoWMi</p>
          <h1 className="text-4xl md:text-7xl lg:text-9xl font-black tracking-tighter uppercase mb-4 leading-[0.9]">
            The Phygital<br/>Era <span className="bg-orange-500 text-black px-3 md:px-5 py-1 rounded-xl inline-block mt-2 border-[3px] border-black shadow-[3px_3px_0px_#000] md:shadow-[5px_5px_0px_#000] rotate-2">Begins</span>
          </h1>
          <p className="text-sm md:text-lg text-neutral-400 font-bold max-w-md mx-auto mt-4 mb-8">
            One premium tee. One scan. <br className="hidden sm:block" /> Connect with anyone, instantly.
          </p>
          
          {/* Mobile-only Premium CTAs */}
          <div className="lg:hidden flex flex-col sm:flex-row gap-4 justify-center items-center mt-6 px-4">
            <button
              onClick={() => {
                const el = document.getElementById('collection');
                if (el) el.scrollIntoView({ behavior: 'smooth' });
              }}
              className="w-full sm:w-auto px-8 py-4 bg-orange-500 text-black font-black text-xs uppercase tracking-widest rounded-xl border-[3px] border-black shadow-[4px_4px_0px_#fff] active:translate-x-[2px] active:translate-y-[2px] active:shadow-none transition-all cursor-pointer"
            >
              Shop Tees
            </button>
            <button
              ...
              className="w-full sm:w-auto px-8 py-4 bg-transparent text-white border-[3px] border-white font-black text-xs uppercase tracking-widest rounded-xl hover:bg-white/10 transition-all cursor-pointer"
            >
              How It Works
            </button>
          </div>

          {/* Desktop-only bounce prompt */}
          <div className="hidden lg:flex flex-col items-center gap-2 mt-12 animate-bounce">
      ...
      {/* Desktop-only secondary scroll sections */}
